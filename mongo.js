...
mongoModule.questions = [
	...
	},
	{
		...
		default: nconf.get('mongo:database') || 'skybb', //modified by lwf
		ask: isUriNotSpecified,
	},
];
...
mongoModule.getConnectionString = function (mongo) {
	...
	if (dbName === undefined || dbName === '') {
		winston.warn('You have no database name, using "skybb"'); //modified by lwf
		mongo.database = 'skybb';
	}
...
mongoModule.init = function (callback) {
	...
	mongoModule.connect(nconf.get('mongo'), function (err, _client) {
		if (err) {
			winston.error('SkyBB could not connect to your Mongo database. Mongo returned the following error', err);
			return callback(err);
		}
		client = _client;
		var db = client.db();
...
mongoModule.checkCompatibilityVersion = function (version, callback) {
	if (semver.lt(version, '2.0.0')) {
		return callback(new Error('The `mongodb` package is out-of-date, please run `./skybb setup` again.'));
	}
