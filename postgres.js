...
postgresModule.questions = [
	...
	},
	{
		...
		default: nconf.get('postgres:database') || 'skybb',//modified by lwf
	},
];
...
postgresModule.getConnectionOptions = function (postgres) {
	...
	if (dbName === undefined || dbName === '') {
		...
		postgres.database = 'skybb';//modified by lwf
	}
...
postgresModule.init = function (callback) {
	...
	db.connect(function (err, client, release) {
		if (err) {
			winston.error('SkyBB could not connect to your PostgreSQL database. PostgreSQL returned the following error: ' + err.message);
			return callback(err);
		}
