...
redisModule.init = function (callback) {
	...
	redisClient = redisModule.connect(nconf.get('redis'), function (err) {
		if (err) {
			winston.error('SkyBB could not connect to your Redis database. Redis returned the following error', err);
			return callback(err);
		}
...
redisModule.checkCompatibilityVersion = function (version, callback) {
	if (semver.lt(version, '2.8.9')) {
		return callback(new Error('Your Redis version is not new enough to support SkyBB, please upgrade Redis to v2.8.9 or higher.'));
	}
	callback();
};
